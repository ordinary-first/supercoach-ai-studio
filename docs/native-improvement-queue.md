...
- [ ] IN_PROGRESS: 로그인 화면 프리미엄 리디자인 (글래스모피즘, 그라디언트, 애니메이션)
- [ ] PENDING: 앱 아이콘/스플래시 실제 디자인 생성 (1024x1024 브랜드 아이콘)
- [ ] PENDING: 온보딩 플로우 개선 (웰컴 → 프로필 → 플랜 선택 단계)

### P1 — 핵심 화면 네이티브 품질
- [ ] PENDING: Goals/비전보드 애니메이션 강화 (셀 리빌, 이미지 줌, 전환 효과)
- [ ] PENDING: Todo 스와이프 제스처 (좌: 삭제, 우: 완료) + 햅틱
- [ ] PENDING: 캘린더 제스처 네비게이션 (좌우 스와이프로 월/주 이동)
- [ ] PENDING: 피드백 화면 차트/그래프 시각화 추가
- [ ] PENDING: Coach Chat 타이핑 인디케이터 + 메시지 버블 애니메이션

### P2 — 전체 앱 폴리시
- [ ] PENDING: 전역 햅틱 피드백 (버튼, 토글, 완료 액션)
