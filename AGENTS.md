...
- Apple-standard Light Mode Redesign & Premium Stabilization completed:
  ...
  - Refactored `theme.css`:
    - Introduced a sophisticated Apple-style Light palette: `#F2F2F7` base, refined blue accents, and high-saturation glassmorphism.
    - Converted hardcoded `.apple-*` classes to use theme-aware variables.
  - Component Theme Awareness:
    - `MindMap.tsx`: Updated `LIGHT_THEME_CONFIG` with premium typography and depth.
    - `ToDoList.tsx`, `TodoSidebar.tsx`, `BottomDock.tsx`, `CalendarView.tsx`: Removed hardcoded dark-mode styles and replaced them with semantic theme variables.
  - Verification:
    - UI remains premium and functional in both Light and Dark modes.
    - `npm run beta:sim:one` passed.
