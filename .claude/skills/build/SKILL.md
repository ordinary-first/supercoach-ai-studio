...
3. `.claude/knowledge/decisions.md` — 관련 아키텍처 결정 확인
...
빌더 에이전트 디스패치:
- `components/*.tsx` → `.claude/agents/builders/component-builder.md`
- `api/*.ts` → `.claude/agents/builders/api-builder.md`
- `hooks/*.ts` → `.claude/agents/builders/hook-builder.md`
- `services/*.ts` → `.claude/agents/builders/service-builder.md`
...
2. 빌드 완료 리포트 출력
3. `.claude/metrics/build-log.jsonl`에 메트릭 기록
